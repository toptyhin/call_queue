...
- Неизвестная/чужая кампания → `404 not_found`; `contact: null` — только отсутствие подходящих контактов, не ошибка.
...
- **Механика SSE**: первая строка — `retry: 3000`; heartbeat `: ping` каждые 15 с молчания; SSE-поле `id:` несут только `chunk`; подключение к терминальному анализу → replay журнала + терминальное событие и закрытие; дисконнект клиента ≠ отмена; конкурентные подписки разрешены.
